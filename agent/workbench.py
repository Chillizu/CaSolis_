...
class Workbench:
    ...
    def build_generate_content(self) -> dict:
        ...
        if style == "profile":
            ...
            lines.append("")
            lines.append("---")
            lines.append(f"_Auto-generated by CaSolis at step {self._step_counter}_")
            content = "\n".join(lines)
            ...
            script_lines = [
                "#!/bin/bash",
                "# CaSolis Experiment Script",
                f"# Generated at step {self._step_counter}",
                f"# Testing unverified hypotheses",
                "",
                "echo '=== CaSolis Experiment ==='",
    ...
    def build_write_content(self) -> dict:
        ...
        if len(system_facts) < 3:
            lines = [f"# CaSolis Snapshot (step {self._step_counter})", ""]
            for k, v in list(self.facts.items())[:5]:
                lines.append(f"{k}={v['value'][:40]}")
            return {"content": "\n".join(lines), "path": "/tmp/casolis_summary.txt", "desc": "简单摘要", "size": len("\n".join(lines))}
        ...
        if style == "json":
            ...
            lines.append("")
            lines.append("---")
            lines.append(f"_Report auto-generated by CaSolis agent at step {self._step_counter}_")
            content = "\n".join(lines)
            path = "/tmp/report.md"
            desc = "Markdown分析报告"
        else:
    ...
    def generate_script(self, script_name: str = "") -> Optional[str]:
        ...
        lines = ["#!/bin/bash", "", "# Auto-generated by CaSolis Workbench", f"# Facts: {', '.join(selected)}", ""]
